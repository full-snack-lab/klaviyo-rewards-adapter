...
export interface TokenStore {
   ...
   */
  runExclusive?<Value>(accountId: string, operation: () => Promise<Value>): Promise<Value>;
}
...
> {
   ...
   * provided.
   */
  async beginAuthorization(input: BeginAuthorizationInput): Promise<PendingAuthorization> {
   ...
   */
  async completeAuthorization(input: CompleteAuthorizationInput): Promise<OAuthTokens> {
 ...
 */
export function parseOAuthCallback(callback: string | URL | URLSearchParams): OAuthCallback {
...
class LockedOAuthApi extends OAuthApi {
  ...
  override async refreshTokens(
    ...
    const pending = (
      this.store.runExclusive ? this.store.runExclusive(accountId, refresh) : refresh()
